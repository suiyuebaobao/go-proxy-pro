  ...
  >
    ...
    <div v-if="step === 1 && !isEdit" class="form-step">
      ...
      <div v-if="showPlatformConfig" class="form-section">
        ...
        <!-- Claude Console / CCR 配置 -->
        <template v-if="form.type === 'claude-console' || form.type === 'claude-ccr'">
    ...
    <div v-if="isEdit" class="form-step">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        ...
        <!-- API 配置 (claude-console / claude-ccr / openai / gemini) -->
        ...
        <div class="form-section">
          ...
          <el-form-item label="模型映射（可选）">
            <el-input
              ...
              type="textarea"
              :rows="3"
              placeholder='{"gpt-4": "claude-3-5-sonnet", "gpt-3.5-turbo": "claude-3-haiku"}'
            />
            <div class="input-tip">
              <i class="fa-solid fa-info-circle"></i>
              JSON 格式，将请求的模型名映射到实际使用的模型
            </div>
          </el-form-item>
        </div>
...
const subplatformMap = {
  claude: [
    ...
    { value: 'claude-ccr', label: 'Claude CCR', desc: 'Claude Code Relay', icon: 'fa-solid fa-link', color: '#9b59b6' },
...
const needsApiKey = computed(() => {
  const type = form.type
  if (['claude-console', 'claude-ccr', 'azure-openai'].includes(type)) return true
...
const showPlatformConfig = computed(() => {
  const type = form.type
  // 直接需要配置的类型
  if (['claude-console', 'claude-ccr', 'bedrock', 'azure-openai'].includes(type)) return true
  // API Key 方式需要配置
  if ((type === 'openai' || type === 'gemini') && form.addType === 'apikey') return true
...
const showEditApiConfig = computed(() => {
  ...
  return ['claude-console', 'claude-ccr', 'openai', 'gemini'].includes(type)
})
...
watch(() => form.type, (newType) => {
  ...
  } else if (['claude-console', 'claude-ccr', 'bedrock', 'azure-openai'].includes(newType)) {
...
function buildSubmitData() {
  ...
  if (form.model_mapping) {
    data.model_mapping = form.model_mapping
  }
